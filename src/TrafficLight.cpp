...
void TrafficLight::cycleThroughPhases()
{
    ...
    while(1){
        ...
        if (elapsedTime >= cycleDur){
            
            if (getCurrentPhase() == TrafficLightPhase::red){setCurrentPhase(TrafficLightPhase::green);}
            else{setCurrentPhase(TrafficLightPhase::red);}
            
            std::cout << "message is sent to Message Queue: " << getCurrentPhase() << "\n";
